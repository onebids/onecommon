...
// ExampleFieldIsolation 展示如何使用字段级租户隔离
func ExampleFieldIsolation() {
	// 示例1：使用默认配置
	config := tenant.NewDefaultFieldDBConfig()
	// 设置DSN
	config.DSN = "user:password@tcp(localhost:3306)/shared_db?charset=utf8mb4&parseTime=True&loc=Local"
	// 设置迁移函数
	config.MigrateFunc = func(db *gorm.DB) error {
		return db.AutoMigrate(&UserWithTenant{})
	}
	...
	// 为租户A创建用户
	userA := UserWithTenant{Name: "Alice"}
	...
	// 创建自定义字段级租户隔离数据库管理器
	customDBManager := tenant.NewFieldDBManager(customConfig)
